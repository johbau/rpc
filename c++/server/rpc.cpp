...
bool RpcServer::handleRequest() {
 ...
 void RpcServer::processMessage(char* buffer, ssize_t msgLen) const {
     auto request = GetHelloRequest(buffer);

     FlatBufferBuilder fbb;

     switch(request->request_type()) {
         case RequestType_GREETING: {
             // Create response message
             std::string originalMsg(reinterpret_cast<const char*>(request->message()->data()),
                                        request->message()->size());

             auto modified = fbb.CreateString("Server received: " + std::string(originalMsg));

             HelloResponseBuilder resp(fbb);
